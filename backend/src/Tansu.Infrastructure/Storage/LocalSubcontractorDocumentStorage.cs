...
public sealed class LocalSubcontractorDocumentStorage : ISubcontractorDocumentStorage
{
    private readonly string _baseRoot;

    public LocalSubcontractorDocumentStorage(IOptions<PhotoStorageOptions> options)
    {
        _baseRoot = options.Value.PhotoStoragePath;
        Directory.CreateDirectory(Path.Combine(_baseRoot, "subcontractor-documents"));
    }
        ...
        CancellationToken ct)
    {
        ...
        var dir = Path.Combine(_baseRoot, "subcontractor-documents", subcontractorId.ToString());
        Directory.CreateDirectory(dir);

        var fileNameOnDisk = $"{documentId:N}{ext.ToLowerInvariant()}";
        var full = Path.Combine(dir, fileNameOnDisk);
        var relative = Path.Combine("subcontractor-documents", subcontractorId.ToString(), fileNameOnDisk);
